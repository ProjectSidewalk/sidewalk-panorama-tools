...
Outcomes, all remembered in `<storage_path>/refetch_log.csv` (a row means permanent, exactly as the two
nightly ledgers do it; anything transient is counted and left unledgered so it retries):
    ...
    undersized    a tile still came back below 512px, so there is nothing to gain by swapping
    too_black     the fresh stitch has more black than a real panorama does; refused
    replaced      swapped in

The first four cost zero HTTP requests, which is what keeps a pass over a large store cheap and makes a
re-run after a completed sweep free even if the ledger is lost.
...
# Outcomes that are a permanent property of the panorama, the store, or Google - ledgered and never
# re-attempted. Everything else that can happen is an exception, which is transient by construction.
PERMANENT_OUTCOMES = ('absent', 'unreadable', 'not_affected', 'already_clean', 'dims_changed',
...
def build_parser():
    ...
    parser.add_argument('--measure', action='store_true',
                        help='Before each swap, record what the re-fetch actually recovered to '
                             '%s. Decodes the stored frame as well as the fresh one, so it roughly doubles '
                             'peak memory - meant for a pilot, not a bulk pass.' % MEASUREMENTS_FILENAME)
    return parser
...
def load_ledger(ledger_path):
    """Every pano id with a permanent outcome already recorded.
    ...
    every future run (#55). An unrecognised status is also skipped, so a ledger written by a newer version
    of this script degrades to re-examining those panoramas - which is safe, because every gate is re-run.
    """
    resolved = set()
    ...
    with open(ledger_path, newline='') as f:
        for row in csv.reader(f):
            if len(row) != 2 or row[0] == 'pano_id' or row[1] not in PERMANENT_OUTCOMES:
                continue
            resolved.add(row[0])
    return resolved
...
def decide_without_fetching(storage_path, record, fixed_after, allow_dims_change):
    ...
    outcome is a permanent status when the panorama needs no request at all, and None when it does; in that
    case fetch_dims is the (width, height) frame to fetch at.
...
def refetch_pano(storage_path, record, fetch_dims, max_black, measure, measurements):
    ...
    Returns a permanent outcome string. Raises for anything transient - a failed tile, a mostly-black stitch,
    a storage error - so the caller counts it and leaves it unledgered, and it retries next run.
    """
    ...
    if measure:
        # Before the swap, because the comparison needs the bytes the swap is about to replace.
        from PIL import Image
...
def refetch_store(storage_path, records, dry_run=False, fixed_after=None, allow_dims_change=False,
    ...
    try:
        ...
        if ledger_file is not None:
            ledger_file.close()
        if measurements:
            _append_measurements(os.path.join(storage_path, MEASUREMENTS_FILENAME), measurements)
...
def main(argv=None):
    ...
    configure_logging(os.path.join(args.storage_path, LOG_FILENAME))
    ...
    # Exit 1 only on transient failures - something is wrong with the network or the store and cron should
    # say so. Every permanent outcome, `gone` included, is this tool working correctly.
    return 1 if counts.get('transient_failures') else 0
