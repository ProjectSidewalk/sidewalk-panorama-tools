...
* `<first-2-chars-of-pano-id>/<pano_id>.depth.npz` — the depth map, stored next to the pano's `.jpg`. Load with numpy:
  ...
  The array shares the JPEG's orientation: column 0 of `d["depth"]` is the leftmost column of the pano image. (streetlevel's decoder delivers the payload x-mirrored relative to the imagery; we flip it back on write, and a contract test pins the upstream behavior — see [#58](https://github.com/ProjectSidewalk/sidewalk-panorama-tools/issues/58). **An artifact with no `format_version` field predates that fix and is horizontally flipped.**) To sample the depth under a label position stored in the database:

  ```python
  col = int(round(pano_x / pano_width * d["depth"].shape[1])) % d["depth"].shape[1]
  row = min(int(round(pano_y / pano_height * d["depth"].shape[0])), d["depth"].shape[0] - 1)
