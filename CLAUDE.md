...
4. **Reports the depth backfill (#43).** `depth_progress()` is the one definition of the figures: *resolved* is `depth_total` of the newest row on which the phase ran (`depth_total > 0` — five zeros is a stand-down or `--skip-depth`, not zero progress), the *rate* is requests summed per night (the queue can run a city twice a night), and the *ETA* is unresolved ÷ rate or `None`. Rule 4 excludes depth minutes (the phase runs to whatever budget it is handed), rule 5 reads `PHASE_COLUMNS` only (field 19 is blank on every older row), rule 6 flags *overlapping* runs rather than same-day ones, and rule 7 warns when three nights pass with no request while work remains.
