...
- **Earned speed persists across runs; back-offs do not.** Every city run is its own process, and measured over the first three production nights every one opened at 1.0 s and ended its 12-minute slot still ramping (1,400 clean requests to the floor, ~590 per slot), so the fleet's real rate was the opening interval and the five largest cities were 250–460 nights out. The interval and clean streak a run *earns* are now written to local disk beside the block latch (`--depth-pace-state`, `gsv.default_pace_state_path`) and the next run opens there, clamped into `[floor, depth_start_interval]`. Only earned speed: `on_pushback` is fed by every network failure, so a persisted back-off would let one DNS blip hand the next 51 cities a 30 s gap — a push-back rewrites the file with the opening interval, a blocked stop forfeits it, and a file older than 24 h, unparseable, non-numeric or `NaN` opens careful (`json.loads` accepts a bare `NaN`, and a NaN interval passes every clamp and reads as "no gap"). The write happens inside a `requests` response hook, so it never raises. What the pacer cannot fix is the slot: the queue spends 12 minutes per city whether or not the city has work.
