...
# The display copy beside every wide pano (#115); idempotent, resumable. NOTHING writes one automatically
# any more (switched off 2026-09-09) - this is the only writer left, and it runs only when you run it.
...
**The whole feature is switched off (2026-09-09) and this is the only writer left.** `downloaders.common.WRITE_DISPLAY_COPIES = False`, so both downloaders and `refetch_panos` write nothing on their own; this script is unaffected, because it runs only when a person runs it. #115's premise was wrong by 2×: Pannellum uploads an equirectangular image as **two half-width textures**, so its ceiling is `2 × MAX_TEXTURE_SIZE` and a device advertising 8192 renders the 16384-wide frames the store holds. Measured demand: 140,599 wide expired panos viewed **29 times in ninety days** against a 6.4 TB backfill, and ~50 users a year who genuinely cannot render 16384. The web app cuts the copy on demand instead (SidewalkWebpage#5256, ~105 MB heap and ~2.0 s inside the JPEG decode), which also retires the "ImageIO can't do this without OOM-killing the JVM" half of the rationale (#5239). **Kept rather than reverted because the margin is exactly zero** — `16384 = 2 × 8192`, GSV has already widened once (13312 → 16384), and this repo is the only place that sees the reported width change. Full reasoning and the numbers: `docs/ops.md`.
...
5. **The guard is at the three automatic callers, not in the primitives.** `gsv._write_display_copy`, `mapillary._write_display_copy` and `refetch_panos._refresh_display_copy` each read `WRITE_DISPLAY_COPIES`; `write_downscaled_sidecar*` never do. Pushing it down into them is the "simplification" that leaves every store-clean assertion green while silently disarming this script — `TestTheSwitch` in `tests/test_downscaled_sidecar.py` exists to fail on exactly that, alongside a test pinning the shipped default to `False` so a flip cannot be accidental.
