...
# Repair pass for the display copy beside every wide pano (#115); idempotent, resumable. See docs/ops.md
...
**downscale_panos.py** (#115) — offline, idempotent, resumable repair pass for the display copy `downloaders.common.write_downscaled_sidecar*` writes: `<pano_id>.w8192.jpg` beside every pano wider than the viewer's cap. Judges each pano from two JPEG headers (its own and its sidecar's), so a finished store decodes nothing; `--max-runtime` leaves the rest counted as `unreached`. The web app serves the sidecar in place of the native file and never cuts one itself — doing so OOM-killed prod JVMs (SidewalkWebpage#5239). Four things about it are load-bearing:
...
3. **A display copy is ~60% of the native file, not a thumbnail** — measured 3.82 MB against 6.08 MB on `samples/sample_pano.jpg` at `DOWNSCALED_JPEG_QUALITY` 85 (2.82 MB at 75). Running the backfill fleet-wide is close to a +60% commitment on the whole store; `--dry-run` gives the count to multiply.
4. **Never fatal in the downloaders**: a failed sidecar is logged and the native pano stays a success, because the native file is already the resume marker.
...
7. **It DOES refresh the #115 display copy** (`_refresh_display_copy`), and must. Every gate above refuses a swap that changes the frame, so a stale sidecar would keep the width its name promises and read as `current` to `downscale_panos`' sweep for ever — the viewer serving a copy of precisely the imagery the pass replaced, with nothing on disk able to tell. Unlike a crop, the sidecar is a pure function of the pano and the raster is in hand at the swap. Never fatal, for a sharper reason than in the downloaders: the swap has **already landed**, so raising would leave the pano unledgered and the next run would spend another ~512 tile requests redoing it.
...
| `<pano_id[:2]>/<pano_id>.w8192.jpg` | Display copy of a pano wider than 8192 px (#115): the viewer's one-texture cap. Written by both downloaders beside the pano and by `refetch_panos` after a swap, repaired by `downscale_panos.py`. **Never list the store's `*.jpg` by hand — call `downloaders.common.walk_store_panos()`**, the one definition of "this file is a panorama"; a walker that forgets `is_downscaled_sidecar()` does not fail, it invents pano ids like `<real id>.w8192` |
