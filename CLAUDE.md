...
7. **Extra passes spend the window on whoever still has work (#43).** Pass 1 is the guarantee and is unchanged; then, while a full slot remains, the cities whose previous run *stopped on its budget* (`stopped_on_budget`: `outcome == 'ok'` and elapsed ≥ budget — exact, because the runner's clock starts after its pano-list fetch, so a fraction would re-run a city that merely finished at minute 11) are run again with `max(slot, remaining / cities_left)`. Measured before this existed: 477 of 690 minutes used, 13 cities complete, the five largest 250–460 nights out. The floor at a slot is load-bearing (5.5-minute shares would zero every image phase under `--min-depth-runtime 6`, and sub-minute ones can kill a city inside its pano-list fetch); a later pass never yields `skipped_deadline` but a crash in one still exits 1; `--single-pass` opts out and `--only` implies it; results carry `pass_number` and `budget_minutes` as defaulted trailing fields so `summarise`'s "N/M cities" stays per-city. The analyzer sums depth requests per night and flags *overlapping* runs, not same-day ones, because of this.
