...
* `0` — the source has nothing for this pano. A permanent verdict, one per source:
  * **GSV** — no imagery at any zoom, or unknowable dimensions.
  * **Mapillary** — a 404, or a record that names the image and carries no original-resolution rendition.
    ...
    record with no rendition is the one that fires in practice. Mapillary is the one source with a
    run-level breaker behind it
  ...
  * **Panoramax** — a 404 **carrying the catalog's own `Feature not found` body**, an item affirming a
    ...
    block that offers no `hd` rendition. Panoramax has **no breaker entry**, so nothing bounds these
    within a run; that is why each rests on the catalog affirming something rather than on a status code
    or a missing key.
