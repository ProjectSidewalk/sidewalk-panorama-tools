...
| The [width tripwire](ops.md#the-width-tripwire): the ceiling's value and that it is not written in terms of the display-copy cap, the 16384/16385 boundary, both channels (`caplog` and `capsys`) each carrying the pointer to the runbook on its own, the helper provably writing nothing (a runtime guard on every write route and a lexical check of its body, which kill mutants as a pair), that a wide pano is still stored byte for byte, and the call site in each of the three downloaders, GSV's before any request | `test_viewer_ceiling.py` |
