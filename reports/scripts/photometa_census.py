...
def resummarize(json_path):
    ...
    A re-fetch census (one carrying `refetch_of`) also gets its `decay` block recomputed against the
    prior it names, looked up beside it, so a reducer added after the re-fetch — `pose_drift` (#114) —
    reaches the committed artifact without a second network pass, and the numbers a report quotes from
    it can be regenerated by this one command. The prior is left untouched: it is the manifest.
    """
    with open(json_path) as f:
        result = json.load(f)
    records = pd.DataFrame(result['records'])
    result['summary'] = summarize(records)
    prior_name = result.get('refetch_of')
    if prior_name:
        with open(os.path.join(os.path.dirname(os.path.abspath(json_path)), prior_name)) as f:
            prior = pd.DataFrame(json.load(f)['records'])
        result['decay'] = decay(prior, records)
...
def pose_drift(joined):
    ...
    affordable image-side tripwire and there should not be one. But 15 of the 19 re-rendered panoramas in
    the pilot had moved - 14 of them with a re-estimated camera pose the fit can name - and pose arrives
    with the photometa request this census already pays for. So pose drift is a re-render proxy at
    metadata cost.

    What it does not catch, stated because a proxy silently under-reporting is worse than no proxy: the
    four panoramas that were re-graded or had their nadir patch replaced without moving. At most 15 of
    19 on the only sample where both are known. It is a lower bound on re-rendering, never a count of it.
    ...
    changed_any = np.zeros(len(alive), dtype=bool)
    measured_any = False
    for axis in POSE_AXES:
        ...
        changed_any[alive.index.isin(pair.index[delta > 0])] = True
    ...
    # The union across axes, beside the per-axis counts, is what lets a reader (and the report's test)
    # tell "the same panoramas moved on every axis" from "as many moved on each": equal per-axis counts
    # that also equal this one are one set, since |A| = |B| = |A u B| forces A = B.
    out['n_changed_any_axis'] = int(changed_any.sum()) if measured_any else None
    return out
...
def print_pose_drift(p):
    ...
    print(f"pose re-estimated (>= {p['threshold_deg']}deg on any axis): "
          ...
          f"({fmt(p['drifted_pct'], '.2f')}%); moved at all on any axis: "
          f"{fmt(p.get('n_changed_any_axis'), 'd')}")
...
def main(argv=None):
    ...
    if args.resummarize:
        ...
        if 'decay' in result:
            print_decay(result['decay'], days=result.get('since_days'))
        return
